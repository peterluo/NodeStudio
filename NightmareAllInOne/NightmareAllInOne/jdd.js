...
function gatherJdProduct(row) {
    ...
    for (var i = 1; i <= total; i++) {
        cate.push(row);
        myScrape.goto(row.ItemUrl + "?page=" + i)
            .wait()
            .evaluate(function () {
                ...
                items.each(function (index, object) {
                    ...
                    urls.push({ name: productName, price: price, remark: remark, url: url, listImage: imageUrl });
                });
                ...
                function (result) {
                    ...
                    for (var j = 0; j < result.length; j++) {
                        //console.log(cate[k].Id + "::" + cate[k].Item + result[j].name + "::" + result[j].price + "::" + result[j].remark + "::" + result[j].url);
